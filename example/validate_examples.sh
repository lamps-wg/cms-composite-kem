...
openssl pkeyutl -decap -in ciphertext.bin -inkey MLKEM768-ECDH-P256-SHA3-256.priv -secret shared_secret_decaps.bin
rm ciphertext.bin
...
cat kek.txt | tr -d '\n' > kek_oneline.txt
openssl kdf -keylen 32 -kdfopt digest:SHA2-256 -kdfopt hexkey:$SHARED_SECRET -kdfopt hexinfo:$KDF_INFO -binary HKDF | xxd -ps | tr -d '\n' > kek_derived_oneline.txt
...
cat cek.txt | tr -d '\n' > cek_oneline.txt
openssl enc -d -aes-256-wrap -K $KEK -iv A6A6A6A6A6A6A6A6 -in encrypted_cek.bin | xxd -ps | tr -d '\n' > cek_decrypted_oneline.txt
